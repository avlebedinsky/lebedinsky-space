...
| POST | `/mood` | все | создать mood-запись пользователя |
| DELETE | `/mood/{id}` | все | удалить mood-запись пользователя |
| GET | `/settings` | все | настройки сайта (тема, gridOrder, hiddenWidgets, widgetSpans, kb*) |
| PUT | `/settings` | admin | обновить настройки сайта |
...
- **MoodWidget** — трекер настроения (1..5) с короткой заметкой. Данные пользователя сохраняются в БД через `/mood`.
- **PixelPetWidget** — анимированный пиксельный питомец в карточке виджета.

Виджеты можно скрывать через AdminPage (секция «Виджеты»). Список скрытых виджетов хранится в `settings.hiddenWidgets` (JSON-массив строк в БД). Доступные ID: `clock`, `weather`, `metrics`, `network`, `docker`, `mood`, `pixel-pet`.

Для каждого виджета можно задать размер в сетке (колонок и строк) через `WidgetForm` в секции «Виджеты». Для карточек сервисов — через `ServiceForm`.
...
Поддерживаются поля `hidden` и `itemLimit` на каждую ленту; `PUT /rss/feeds/{id}` позволяет изменять URL, заголовок, видимость и лимит элементов.

### База знаний

Роуты: `GET /knowledge/tree` и `GET /knowledge/file?path=...`.

- Источник — GitHub-репозиторий из user settings (`kbRepoURL`, `kbGithubToken`).
- В API кэшируются дерево и файлы на 5 минут.
- Во фронтенде рендер markdown идёт через `react-markdown` + `remark-gfm` + `rehype-highlight`.
- Ограничение видимости по верхнеуровневым папкам настраивается через `kbAllowedFolders`.
...
- **Скрытие виджетов** — поле `hidden_widgets` (JSON-массив) в `user_settings`; управляется через AdminPage (секция Виджеты), хранится в `settings.hiddenWidgets`
- **Размер виджетов/карточек** — управляется через AdminPage: `WidgetForm` для виджетов, `ServiceForm` для карточек сервисов
- **Mood tracker** — хранится в таблице `mood_entries`; записи изолированы по пользователю
- **База знаний** — пользовательские поля `kb_repo_url`, `kb_github_token`, `kb_allowed_folders` в `user_settings`; токен не возвращается фронтенду
...
- **ColorPicker** — переиспользуемый компонент `src/components/ColorPicker.tsx`: кликабельный свотч + hex-значение поверх скрытого нативного `input[type=color]`
